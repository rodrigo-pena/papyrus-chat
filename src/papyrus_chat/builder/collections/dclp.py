"""Adapter extracting records from DCLP EpiDoc XML (SPEC 6.3, 3.1).

Records without edition text stay discoverable as metadata-only documents.
`display_text` keeps visible uncertainty signals (brackets for supplied text,
[...] for gaps); `search_text` is normalized separately; unsupported elements
are recorded as warnings instead of being silently invented or dropped.
"""
...
def _textpart_label(element: _etree._Element) -> str | None:
    ...
    for ancestor in element.iterancestors():
        if ancestor.get("type") != "textpart":
            break
        if ancestor.get("n"):
            labels.append(ancestor.get("n") or "")
    if element.get("n"):
